                ...
                if (openFileDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    for (int i = 0; i < files.Length; i++)
                    {
                        ...
                        try
                        {
                            ...
                            dt1 = dt.AddSeconds(header.start_time); dt1 = dt1.ToLocalTime();
        ...
        private void dataGridView_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            unsafe
            {
                ...
                if (e.ColumnIndex == 7)
                {
                    ...
                    try
                    {
                        fs = new System.IO.FileStream(textBoxDir.Text + "\\" + dataGridView.Rows[e.RowIndex].Cells[0].Value, System.IO.FileMode.Open);
                        byte[] bytes = new byte[sizeof(HDR_HEADER)];
                        fs.Read(bytes, 0, sizeof(HDR_HEADER));
                        System.Runtime.InteropServices.Marshal.Copy(bytes, 0, ptr, sizeof(HDR_HEADER));
                        fs.Close(); fs = null;
